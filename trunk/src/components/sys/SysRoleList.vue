        ...
        <el-table :data="arrList" size="medium" stripe highlight-current-row v-loading="loading" style="width: 100%;"
            ...
            <el-table-column label="操作" width="250" fixed="right">
                <template slot-scope="scope">
                    ...
                    <el-button type="primary" size="mini" @click="handleSetRes(scope.$index, scope.row)">设置权限</el-button>
...
<script>
    import {URI_SYSROLE} from '../../const/uri';
    ...
    export default {
        ...
        methods: {
            ...
            //设置权限
            handleSetRes(index, row) {
                this.$router.push('/sysRoleResourceList/' + row.id + '/' + row.name);
            },
            //删除
            handleDel(index, row) {
                this.$confirm('确认删除['+row.name+']吗?', {
                    ...
                    this.$http.delete(URI_SYSROLE + '/' + row.id).then((res) => {
            ...
            editSubmit() {
                this.$refs.editForm.validate((valid) => {
                    if (valid) {
                        if (this.editForm.id) {
                            this.$http.put(URI_SYSROLE + '/' + this.editForm.id, this.editForm).then((res) => {
                                this.$message.success(res.data.message);
                            ...
                            this.$http.post(URI_SYSROLE, this.editForm).then((res) => {
                                this.$message.success(res.data.message);
                                this.$refs['editForm'].resetFields();
                                this.editFormVisible = false;
                                this.initData();
